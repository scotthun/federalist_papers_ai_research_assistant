...
- source_spec: `docs/implementation/spec-5-1-quill-widget-launcher-panel-streaming-ask.md`
  summary: Candidate follow-up story -- genuine token-level LLM streaming for the confident tier, instead of this story's client-perceived relay (full answer generated + citation-verified synchronously, then replayed to the browser at a fixed per-word pace).
  evidence: Manual product review after this story shipped: the current streaming still has a silent wait for the *entire* answer to generate before any text appears, then a fairly mechanical constant-pace playback -- rougher than a real incremental stream. This is an inherent trade-off, not an oversight: citations must never render before server-side citation verification completes (this story's/Epic 3's Boundaries), and today's confident-tier generation is one `generateStructuredOutput` call that returns the whole `{ answer, citations }` object at once, so there's nothing to verify or stream incrementally until it's fully done. A real fix would mean changing the LLM call itself (e.g. streaming raw text tokens while deferring citation extraction/verification to a trailing structured pass, or a provider that supports partial/streaming structured output) -- a genuine `AskService`/`AIProvider` change, not a UI-layer tweak, so it deserves its own story rather than a patch to this one. Revisit as a dedicated Epic 5 (or later) story if faster perceived time-to-first-word becomes a priority.

- source_spec: `docs/implementation/spec-4-1-deploy-vercel-neon.md`
  summary: `bootstrap()`'s actual wiring of `resolveCorsOrigin(process.env)` into `app.enableCors(...)` is untested -- only the pure `resolveCorsOrigin` function is unit-tested, so a regression at the call site (e.g. the `if` guard inverted, or `enableCors()` called with no options at all, reflecting every origin) would ship without any test failing.
  evidence: Verification-gap review confirmed no e2e/integration harness exists for `apps/api` to reach this. Not treated as a blocking patch: `parsePort` (the same file's existing, accepted precedent) has an identical gap -- its own call site inside `bootstrap()` is likewise never verified, only the pure function is. This story's CORS wiring follows that same established, already-accepted convention rather than introducing a new, worse gap. Revisit if `bootstrap()` ever grows complex enough to warrant its own integration test (e.g. via a mocked `NestFactory`).

- source_spec: `docs/implementation/spec-4-1-deploy-vercel-neon.md`
  summary: `createDataSourceOptions`'s new `extra: { max: 5 }` sets no `idleTimeoutMillis`/`connectionTimeoutMillis` -- under Fluid compute's concurrent-warm-instance model, an idle-but-warm `apps/api` instance could hold pooled connections against Neon's PgBouncer indefinitely with no idle timeout.
  evidence: Blind-hunter review finding. Speculative future tuning, not a confirmed live problem -- the research doc this story is grounded in didn't flag this as required, only `extra.max` sizing. Revisit once real production connection behavior can be observed (e.g. Neon's connection-count dashboard) rather than tuning blind.
