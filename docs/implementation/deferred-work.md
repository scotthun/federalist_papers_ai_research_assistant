...
- source_spec: `docs/implementation/spec-gh-24-ai-provider-langchain.md`
  summary: `ARCHITECTURE-SPINE.md`'s stack table still pins `@langchain/core` at `1.2.8`, but `langchain@1.5.10`'s own `peerDependencies` require `^1.2.9` -- `1.2.8` cannot actually be installed alongside it, so the architecture doc and the real dependency graph now disagree.
  evidence: Confirmed via `npm install` during GH-24's implementation (recorded in that spec's Change Log) and independently flagged by code review. Out of GH-24's file scope (`libs/ai` only per that spec's Boundaries). One-line doc fix: bump the stack table entry to `1.2.9`.
